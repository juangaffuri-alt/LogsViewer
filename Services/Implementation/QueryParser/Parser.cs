...
using LogsViewer.Services.Implementation.QueryParser.Nodes;
using Piglet.Parser;
...
public class Parser
{
    const int MaxQueryLength = 10000;
    ...
    private static IParser<IAstNode> CreateParser()
    {
        ...
        ITerminal<IAstNode> propertyTerminal = configurator.CreateTerminal("[0-9a-zA-Z_][0-9a-zA-Z_-]*|\\[[0-9a-zA-Z_ ][0-9a-zA-Z_ -]*\\]", ParseProperty);
    ...
    public static IAstNode SimpleParse(string input)
    {
        ...
        if (input.Length > MaxQueryLength) throw new Area52QueryException($"Query exceeds maximum length of {MaxQueryLength}");
